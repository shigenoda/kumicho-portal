...
## 13次実装：メールアドレス登録改善・問い合わせシステム・返信待ちキュー（完了）

### DBスキーマ拡張
- [x] 問い合わせテーブル（inquiries）の追加
- [x] 問い合わせ返信テーブル（inquiry_replies）の追加
- [x] DBマイグレーション実行

### 問い合わせ API
- [x] 問い合わせ作成 API（createInquiry）
- [x] 問い合わせ一覧取得 API（getInquiries）
- [x] 問い合わせ返信 API（replyToInquiry）
- [x] 返信待ちキュー取得 API（getPendingInquiries）
- [x] 問い合わせ詳細取得 API（getInquiryDetail）
- [x] 通知機能（問い合わせ時に組長へメール）

### メールアドレス登録画面の改善
- [x] 会話別メールアドレス表示（自分のは表示、他は伏せ字）
- [x] 会話別メールアドレス登録フォーム（EmailSettings.tsx改善）
- [x] 登録状态の表示（登録済み/未登録）
- [x] 他の世帯の登録状态を確認できるUI

### 問い合わせ作成画面
- [x] 問い合わせ作成フォーム（InquiryForm.tsx）
- [x] タイトル・内容・カテゴリ入力
- [x] 組長への通知メール送信
- [x] 作成完了メッセージ

### 返信待ちキュー画面
- [x] 返信待ちキュー一覧ページ（InquiryQueue.tsx）
- [x] 未返信の問い合わせ表示
- [x] 返信フォーム
- [x] 返信済み/未返信のステータス表示
- [x] 問い合わせ詳細表示
...
### テスト・チェックポイント
- [x] 動作確認
- [x] テスト作成（inquiry-system.test.ts）
- [x] 全70テスト合格
